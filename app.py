    ...
    if form.validate_on_submit():
        ...
        if User.authenticate(g.user.username, form.password.data):
            try:
                ...
                db.session.commit()
                # Redirect to user profile and flash message
                return redirect(url_for('users_show', user_id=g.user.id))
            
            except IntegrityError:
                # Rollback if username/email is already taken
                db.session.rollback()
                # Flash message if username/email is already taken
                flash("Username/Email already taken", 'danger')
                return render_template('users/edit.html', form=form)
        
        # Flash message if password is invalid
...
def change_password():
    ...
    # Validate form
    if form.validate_on_submit():
        # Check if current password is correct, if so, change password
        if g.user.change_password(form.current_password.data, form.new_password.data):
            flash("Password successfully changed!", "success")
            return redirect(url_for("users_show", user_id=g.user.id))
        else:
            # Flash message if current password is incorrect
            flash("Current password is incorrect.", "danger")

    return render_template("/users/change-password.html", form=form, user_id=g.user.id)
